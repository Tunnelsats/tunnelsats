...
In v2 we changed the network architecture compared to v1 where all traffic was directed via the VPN except tor or ssh. This approached resulted in a lot of exception for different users. For example if you were running thunderhub on your node with v1 you were not able to access it via the external clearnet. In v2 we are now isolating only lightning traffic via the VPN meaning that all traffic not routed via your local proxy and not destined for your local network will be directed through the tunnel. This means apps like thunderhub which run locally on your node are not tunneled and are accessible from the external clearnet. Nothing changes for your setup execpt for the lightning process. You will have no problems accessing your node from the external clearnet via ssh. So keep in mind that in case you want to access the grpc|REST interface form the external clearnet it will not be possible. In this case use services like ZeroTier which let you access your node as it would be in your local network. Normally accessing your nodes grpc|REST api from the external clearnet shouldn't be a gerneral use case, its recommended to access the api via the local network or on the same computer resulting in better efficiency.
