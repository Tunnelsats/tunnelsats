...
SIGNATURE=$(docker exec -it $LIGHTNINGCONTAINER lncli signmessage "$NOW" | jq -r .signature)

docker run -it --rm --net=docker-tunnelsats curlimages/curl -s -X POST -H "Content-Type: application/json" -d '{
